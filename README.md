...
The rules in `slice.csv` and `firewall.csv` must be in accordance with the standard defined in `config.json`. In steps 2 and 3, it will be shown how you can test the `verifire.py` script against pre-defined test cases, as well as your own test cases built using scripts found in the `bench2csv/` directory.
...
In order to build your own test cases, you will need to utilize the _bench2csv_ scripts which can be found under `verifire/bench2csv`, in addition to the set of 29,000 unique firewall rules found at `verifire/bench2csv/29krules.csv`. `29krules.csv` was generated using _classbench-ng_, a firewall generation tool which can be found [here](https://github.com/classbench-ng/classbench-ng). Downloading this repository and generating your own firewall rulesets isn't required, however if you choose to do this, you will need to convert the rules into CSV format (the rules output by _classbench-ng_ are tab-deliminated), and it must be kept in mind that running the `classbench` to generate a set of "n" rules will likely result in less than "n" rules actually being generated.

Recall that in order to build a satisfiable/unsatisfiable firewall policy, you will need to provide a discard slice as input. To build a discard slice, you will need to run the following command to output `slice.csv` to your current working directory. It's assumed that `verifire/` is in your current working directory.
```
$ python3 verifire/bench2csv/bench2csv_slice.py verifire/config.json verifire/bench2csv/29krules.csv 100 >> slice.csv
```

Note that the discard slice generated from the above command will have 100 rules, although you can swap 100 with however many rules you would like the slice to have. Now, to build an associated firewall policy, you need to decide if you would like the resulting boolean formula to be unsatisfiable, or to be satisfiable. If your choice is unsatisfiable, then you will need to use the `verifire/bench2csv/bench2csv_unsat.py` script. If your choice is satisfiable, then you will need to use the `verifire/bench2csv/bench2csv_sat.py` script. Both of these scripts take only a discard slice as input (specifically, the discard slice that was generated by the previous command).

To build the firewall policy (for example, a policy that results in an unsatisfying boolean assignment), you can run the following script, where your `slice.csv` is in your current working directory. The resulting firewall policy will be approximately the same size as the input slice with respect to the number of rules. 
```
